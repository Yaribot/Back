...
<body>
    <div class="container text-center">
        ...
        elseif(isset($_COOKIE['pays'])) //on entre dans le elseif dans le cas ou un cookie à été créer et que par exemple nous revenons sur le site 3 mois plus tard, la valeur du cookie sera afficher
        {
            $pays = $_COOKIE['pays'];
        }
        else // on entre dans le cas else lors de la première visite dur le site,  lorsqu'aucun cookie n'est créer
        {
            $pays = 'fr';
        }
        //-----------------------------------------------------------------------------------------
        ...
        setcookie("pays", $pays, time()+$un_an); // permet de créer un fichier cookie qui est conservé côté client, c'est à dire sur l'ordinateur de l'internaute 
        // 3 arguments : nom du cookie / valeur du cookie / durée de vie du cookie
        echo'<pre>'; print_r($_COOKIE); echo'</pre>';

        //Un cookie est un fichier qui est conservé côté client, c'est à dire sur le pc du navigateur, il contient des données non sensibles (derniers articles consultés, langue préféré du site etc...)
