...
SELECT num_chambre FROM chambres WHERE equipement LIKE '%television%';
...
SELECT AVG(prix/nb_pers) FROM chambres WHERE equipement LIKE '%television%';
SELECT prix/nb_pers FROM chambres WHERE equipement LIKE '%television%';
-- Les numéros des chambres et le numéro des clients ayant réservé des chambres pour le 09/02/2004.
SELECT num_chambre, num_client FROM chambres WHERE date_arrive = '2019-03-11';
-- Les numéros des chambres coûtant au minimum 80 Euro ayant un bain et volant au maximum 120 Euro.
SELECT num_chambre, prix FROM chambres WHERE confort LIKE '%salle_de_bain%' AND prix BETWEEN 80 AND 150;
SELECT num_chambre, prix FROM chambres WHERE confort LIKE '%salle_de_bain%' AND prix >= 80 <= 150;
...
SELECT nom FROM clients AS c INNER JOIN reservation AS r WHERE c.num_client = r.num_client;
-- Jointure entre les tables client et réservation avec INNER JOIN
SELECT nom FROM clients INNER JOIN reservation WHERE c.num_client = r.num_client;